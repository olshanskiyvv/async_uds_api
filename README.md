...
Текст сообщения об ошибке, который UDS возвращает в ответе, тоже может
содержать эхо `phone`/`uid`/`code` — иногда в нормализованном виде,
отличном от того, что было передано в запросе (например, без `+` или в
другом регистре). Поэтому текст сообщения (и в поле `message` события
`uds.error`, и в тексте выброшенного исключения) прогоняется через
структурное редактирование `redact_message`: маскируются участки,
похожие на телефон (7–15 цифр подряд, опционально с `+`), участки,
похожие на UUID, и нормализованные формы значений `phone`/`uid`/`code`
из самого запроса — независимо от форматирования и регистра в ответе
сервера. Короткие значения (короче 5 символов после нормализации, а
также цифровые последовательности из 6 и менее цифр) не редактируются,
чтобы не портить остальной текст сообщения.

`redact_message` и `mask_url` (см. ниже) экспортируются из
`async_uds_api` наравне с `mask_value`/`mask_params` — ими можно
пользоваться и вне библиотеки, например в собственных логах.
...
URL без query-строки в лог попадает без изменений.
